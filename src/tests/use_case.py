...
random_string = lambda x: ''.join(random.choices(string.ascii_lowercase + string.digits, k=x))
...
class UseCase:
    ...
    def create_class(self, class_data):
        ...
        class_data['students'] = [ObjectId(student_id) for student_id in class_data['students']]
    ...
    def serialize_object(self, obj):
        ...
        for field in obj:
            if '_id' not in field:
                ...
                elif table and isinstance(obj[field], list) and all(isinstance(elem, ObjectId) for elem in obj[field]):
    ...
    def set_mark(self, journal_id, new_mark):
        self.journal.update_one({'_id': ObjectId(journal_id)}, {'$set': {'mark': new_mark}})

    def create_new_work(self, work_data):
        assert all(work_data.get(param) for param in ['subject_id', 'comment', 'date', 'type'])
        ...
        if work_data.get('student_id'):
            ...
            self.journal.insert_many([
                ...
                } for student_id in self.classes.find_one({'_id': ObjectId(work_data.get('class_id'))}).get('students')
            ])
...
print(test.auth_user({'username': student['username'], 'password': student['password']}))
